...
```go
import spacetraders "github.com/grantmd/spacetraders-mcp/spacetraders"
```
...
For using other server than the one defined on index 0 set context value `spacetraders.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `spacetraders.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `spacetraders.ContextOperationServerIndices` and `spacetraders.ContextOperationServerVariables` context maps.
...
```go
auth := context.WithValue(context.Background(), spacetraders.ContextAccessToken, "BEARER_TOKEN_STRING")
